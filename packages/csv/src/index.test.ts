...
const KPI_HEADER =
  "Monat,Gesellschaft,KPI_Einkaufsvolumen_EUR,KPI_Einsparung_Monat_EUR,KPI_Einsparquote_Prozent,KPI_Zeitersparnis_Std,KPI_RFQs_Abgeschlossen,KPI_Aktive_Lieferanten,KPI_Datenqualitaet_Prozent,KPI_Aktive_Nutzer,KPI_Materialquote_Prozent";

describe("parseKpiCsv", () => {
  it("parst eine gültige, komma-getrennte Datei", () => {
    const csv = `${KPI_HEADER}\n2026-09,Maximator,2900000,15000,1.0,32,2,85,78,6,30`;
  ...
  it("erkennt Semikolon-Trennzeichen und normalisiert Komma-Dezimalwerte (deutscher Excel-Export)", () => {
    ...
    const csv = `${header}\n2026-09;HAZEMAG;1.190.000;0;0,0;3;0;40;40;2;40`;
    const { rows, errors } = parseKpiCsv(csv);
    expect(errors).toEqual([]);
    expect(rows).toHaveLength(1);
  ...
  it("lehnt ungültige Monatsformate ab", () => {
    const csv = `${KPI_HEADER}\n2026-13,Maximator,1,1,1,1,1,1,1,1,1`;
    const { rows, errors } = parseKpiCsv(csv);
    expect(rows).toEqual([]);
...
describe("parseKpiCsv – Forecast-/Budget-Spalten", () => {
  ...
  it("parst vorhandene Forecast- und Budget-Werte", () => {
    const csv = `${HEADER_WITH_FC_BU}\n2026-09,Maximator,2900000,2930000,3020000,15000,1.0,32,2,85,78,6,7,8,30`;
    const { rows, errors } = parseKpiCsv(csv);
    expect(errors).toEqual([]);
  ...
  it("lehnt nicht-numerische Forecast-Werte ab", () => {
    const csv = `${HEADER_WITH_FC_BU}\n2026-09,Maximator,2900000,abc,3020000,15000,1.0,32,2,85,78,6,7,8,30`;
    const { rows, errors } = parseKpiCsv(csv);
    expect(rows).toEqual([]);
